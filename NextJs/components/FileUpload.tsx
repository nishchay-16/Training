...
const FileUpload: React.FC<FileUploadProps> = ({ onClose }) => {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (duplicateFiles.length > 0) {
      ...
      const filesWithPreview = filteredFiles.map(file => Object.assign(file, { preview: URL.createObjectURL(file) }));
  ...
  const handleDeleteClick = (file: FileWithPreview, event: React.MouseEvent<HTMLButtonElement>) => {
